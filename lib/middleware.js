...
service.get = (req, res) => {
    if (!req.params) return errorRes(res, 'unknown')

    if (!req.params.service) {
        Service.find({ host: req.params.host }, (err, services) => {
            if (err) return errorRes(res, err)
            res.send(services)
        })
    }
    ...
    Service.findOne(
        { host: req.params.host, name: req.params.service },
        (err, service) => {
            if (err) return errorRes(res, err)
            res.send(service)
        }
...
service.put = (req, res) => {
    ...
    if (req.body.host != req.params.host ) 
        return errorRes(res, 'mismatch hostname in json/uri')

    Service.findOne({ name: req.body.name, host: req.body.host }, (err, service) => {
        if (err || !service) {
...
service.del = (req, res) => {
    if (!req.params || !req.params.host || !req.params.service) 
        return errorRes(res, 'no params?')
    Service.remove({ host: req.params.host, name: req.params.service }, {}, (err, count) => {
        if (err || count < 1) return errorRes(res, err || 'unknown err')
        okRes(res, `successfully deleted ${req.params.service} on ${req.params.host}`)
    })
}
