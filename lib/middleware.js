...
var STATUS, Host, Service, Auth, TimeSeries, logger, allHosts
...
var rssfeed = (req, res) => {
    Auth.findOne({ key: req.params.secret }, (err, key) => {
        ...
        _.keys(STATUS).map(host => {
            _.keys(STATUS[host]).map(service => {
                if (STATUS[host][service].code == 'OK')
                    return

                feed.addItem({
                    title: `${host} - ${service}`,
                    description: STATUS[host][service].output,
                    date: STATUS[host][service].lastCheck,
                    link: `${config.http.hostname}:${config.http.port}/#/`
                })
            })
        })
...
auth.test_fixed_key = (req, res, next) => {
    if (!req.cookies && !req.cookies.key && !req.get('api-key'))
    ...
    if (req.cookies.key === process.env.API_KEY) {
        req.authenticated = true
...
auth.test_key = (req, res, next) => {
    ...
    Auth.findOne({ key: req.get('api_key') }, (err, key) => {
        if (err || !key) {
            Auth.findOne({ key: req.cookies.key }, (err, key) => {
                if (err || !key) 
...
module.exports = (_STATUS, _Host, _Service, _Auth, _logger, _TimeSeries) => {
    STATUS = _STATUS
    Host =_Host
    Service = _Service
    Auth = _Auth
