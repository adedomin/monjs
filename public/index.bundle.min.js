                ...
                <span class="select">
                  <select id="filterSeries-select"
                   oninput=${e=>send("filterSeriesChange",e.target.value)}>
                    ${(()=>{var uniqs=_.map(_.uniqBy(state.timeseries,"measure"),(uniq=>{return uniq.measure}));if(uniqs.length==1&&state.filterSeries!=uniqs[0])return send("filterSeriesChange",uniqs[0]);if(!state.filterSeries||uniqs.indexOf(state.filterSeries)<0)return send("filterSeriesChange",uniqs[0]);return uniqs.map((uniq=>html`
